  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">{t("O'quv mavzularini o'rganing")}</p>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
            {(topics || []).map((topic, i) => (
              <motion.div
                ...
                className={`rounded-xl border overflow-hidden cursor-pointer transition-all hover:shadow-elevated ${selectedId === topic.id ? "border-primary ring-1 ring-primary" : "border-border"
                  } bg-card shadow-card`}
              >
                {topic.image_url && (
                ...
                <div className="p-4">
                  <h4 className="font-display font-semibold text-foreground text-sm">{t(topic.title)}</h4>
                  {topic.description && (
                    <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{t(topic.description)}</p>
                  )}
                </div>
              </motion.div>
            ))}
        ...
        {selected && (
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-lg font-display font-bold text-foreground">{t(selected.title)}</h2>
                {selected.description && (
                  <p className="text-sm text-muted-foreground">{t(selected.description)}</p>
                )}
              </div>
            </div>
            {selected.image_url && (
            ...
            <div className="bg-muted/30 border-l-4 border-success rounded-r-lg p-4 text-sm text-foreground whitespace-pre-wrap leading-relaxed">
              {t(selected.content || "Bu mavzu uchun hali kontent qo'shilmagan.")}
            </div>
            <div className="mt-4">
