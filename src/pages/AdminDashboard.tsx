...
export default function AdminDashboard() {
  ...
  const { data: top20 } = useQuery({
    ...
    queryFn: async () => {
      ...
      return sorted.map((s, i) => ({ ...s, rank: i + 1, name: map[s.user_id] || t("Noma'lum") }));
    },
  });
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-display font-bold text-foreground">{t("Boshqaruv paneli")}</h1>
        <p className="text-sm text-muted-foreground">{t("Avtomaktab tizimining umumiy ko'rinishi")}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        <StatCard title={t("O'quvchilar")} value={stats?.students || 0} icon={Users} variant="primary" />
        <StatCard title={t("O'qituvchilar")} value={stats?.teachers || 0} icon={GraduationCap} variant="success" />
        <StatCard title={t("Biletlar")} value={stats?.tickets || 0} icon={FileText} variant="warning" />
        <StatCard title={t("Mavzular")} value={stats?.topics || 0} icon={BookOpen} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <h3 className="font-display font-semibold text-foreground mb-4">{t("So'nggi test natijalari")}</h3>
          <div className="space-y-3">
              ...
              <div key={r.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("Bilet")} #{r.tickets?.ticket_number} · {r.correct_answers}/{r.total_questions} {t("to'g'ri")}
                  </p>
                </div>
            ...
            {(!recentResults || recentResults.length === 0) && (
              <p className="text-sm text-muted-foreground text-center py-4">{t("Hali natijalar yo'q")}</p>
            )}
          </div>
        </motion.div>
        ...
        >
          <h3 className="font-display font-semibold text-foreground mb-4 flex items-center gap-2">
            <Trophy className="w-4 h-4 text-warning" /> {t("Top 20 — eng faol o'quvchilar")}
          </h3>
          <div className="space-y-2">
            {(top20 || []).map((s: any) => (
              <div key={s.user_id} className="flex items-center gap-3 p-2.5 rounded-lg bg-muted/30">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${s.rank <= 3 ? "bg-warning/20 text-warning" : "bg-muted text-muted-foreground"
                  }`}>
                  {s.rank <= 3 ? ["🥇", "🥈", "🥉"][s.rank - 1] : s.rank}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground truncate">{s.name}</p>
                  <p className="text-xs text-muted-foreground">{s.count} {t("ta test")} · {t("O'rtacha")} {s.avg}%</p>
                </div>
            ...
            {(!top20 || top20.length === 0) && (
              <p className="text-sm text-muted-foreground text-center py-4">{t("Hali ma'lumotlar yo'q")}</p>
            )}
          </div>
        </motion.div>
      </div>
